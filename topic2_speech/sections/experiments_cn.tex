\section{实验结果与比较}

\subsection{短时 FFT 模板方法}

先看短时 FFT 模板方法。由于任务是单标签四分类，完全匹配准确率是更直接的指标，按标签位准确率只作为辅助参考。

这个方案不再只用四个代表案例音频当模板，而是先对训练集同类样本的最大能量块求平均，再在选定块长后用训练集与验证集重新估计最终模板。块长在 3--6 帧之间由验证集选择，最终取 5 帧。由于数据覆盖 300 个词，跨词和说话人差异较大，单纯依靠 FFT 幅度谱相关还不足以完全区分四类词首辅音，但它保留了清楚的频谱解释。

\begin{table}[H]
    \centering
    \caption{短时 FFT 模板方法的主指标}
    \label{tab:strict-main}
    \begin{tabular}{lccccc}
        \toprule
        方法 & label-wise acc. & exact-match acc. & macro-F1 & micro-F1 & samples-F1 \\
        \midrule
        短时 FFT 模板方法 & 0.6327 & 0.2655 & 0.2439 & 0.2655 & 0.2655 \\
        \bottomrule
    \end{tabular}
\end{table}

\begin{table}[H]
    \centering
    \caption{短时 FFT 模板方法的分标签 F1}
    \label{tab:strict-per-label}
    \begin{tabular}{lcccc}
        \toprule
        方法 & /g/ & /b/ & /d/ & /z/ \\
        \midrule
        短时 FFT 模板方法 & 0.217 & 0.163 & 0.243 & 0.353 \\
        \bottomrule
    \end{tabular}
\end{table}

图~\ref{fig:strict-score-dist} 进一步给出了四个标签的得分分布。正样本和负样本已经有一定分离，但重叠仍然明显，说明仅依靠短时 FFT 模板还不足以稳定完成四分类。

\begin{figure}[H]
    \centering
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/mswc_course_gbdz_75w_cap40_5_5/strict_course/score_distribution_g.png}
        \small /g/
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/mswc_course_gbdz_75w_cap40_5_5/strict_course/score_distribution_b.png}
        \small /b/
    \end{minipage}

    \vspace{0.5em}
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/mswc_course_gbdz_75w_cap40_5_5/strict_course/score_distribution_d.png}
        \small /d/
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/mswc_course_gbdz_75w_cap40_5_5/strict_course/score_distribution_z.png}
        \small /z/
    \end{minipage}
    \caption{短时 FFT 模板方法在四个标签上的得分分布。}
    \label{fig:strict-score-dist}
\end{figure}

\subsection{基于 AI 的重实现方法对照}

在短时 FFT 模板方法之外，本文进一步给出基于 AI 的重实现方法。它不是简单更换分类器，而是重做前两步：先用 log-mel 与能量变化得到更稳的词首局部谱，再用正负模板差分和自相关完成检测。

\begin{table}[H]
    \centering
    \small
    \setlength{\tabcolsep}{4pt}
    \caption{短时 FFT 模板方法与基于 AI 的重实现方法的整体比较}
    \label{tab:course-compare}
    \begin{tabular}{lccccc}
        \toprule
        方法 & label-wise acc. & exact-match acc. & macro-F1 & micro-F1 & samples-F1 \\
        \midrule
        短时 FFT 模板方法 & 0.6327 & 0.2655 & 0.2439 & 0.2655 & 0.2655 \\
        基于 AI 的重实现方法 & 0.7513 & 0.5026 & 0.4974 & 0.5026 & 0.5026 \\
        \bottomrule
    \end{tabular}
\end{table}

\begin{table}[H]
    \centering
    \caption{短时 FFT 模板方法与基于 AI 的重实现方法的分标签 F1}
    \label{tab:course-per-label}
    \begin{tabular}{lcccc}
        \toprule
        方法 & /g/ & /b/ & /d/ & /z/ \\
        \midrule
        短时 FFT 模板方法 & 0.217 & 0.163 & 0.243 & 0.353 \\
        基于 AI 的重实现方法 & 0.464 & 0.359 & 0.429 & 0.737 \\
        \bottomrule
    \end{tabular}
\end{table}

基于 AI 的重实现方法的提升主要来自两点：一是把搜索范围更精确地收到了词首附近，二是把局部窗中的正负模板差分显式纳入了得分。/z/ 的提升最大，说明持续摩擦音在 log-mel 局部模板和自相关线索里更稳定；/b/ 和 /d/ 的重叠仍然明显，说明短促爆破和对齐误差仍会削弱模板证据。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/mswc_course_gbdz_75w_cap40_5_5/letter_examples.png}
    \caption{基于 AI 的重实现方法对应的四个代表样本。}
    \label{fig:ext-examples}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.90\textwidth]{figures/mswc_course_gbdz_75w_cap40_5_5/template_grid.png}
    \caption{基于 AI 的重实现方法使用的平均 log-mel 模板视角。它展示的是每类训练样本的平均谱图，不等同于正负差分模板，但能帮助观察四类词首能量分布。}
    \label{fig:ext-template-grid}
\end{figure}

图~\ref{fig:course-score-dist} 单独给出了基于 AI 的重实现方法四个标签的测试得分分布。相比短时 FFT 模板方法，/z/ 的正负样本分离最明显；/g/、/b/、/d/ 仍然存在明显重叠，这也解释了为什么基于 AI 的重实现方法提升明显，但没有完全消除爆破音之间的混淆。

\begin{figure}[H]
    \centering
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/mswc_course_gbdz_75w_cap40_5_5/course_filterbank_corr/score_distribution_g.png}
        \small /g/
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/mswc_course_gbdz_75w_cap40_5_5/course_filterbank_corr/score_distribution_b.png}
        \small /b/
    \end{minipage}

    \vspace{0.5em}
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/mswc_course_gbdz_75w_cap40_5_5/course_filterbank_corr/score_distribution_d.png}
        \small /d/
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/mswc_course_gbdz_75w_cap40_5_5/course_filterbank_corr/score_distribution_z.png}
        \small /z/
    \end{minipage}
    \caption{基于 AI 的重实现方法在四个标签上的得分分布。}
    \label{fig:course-score-dist}
\end{figure}
