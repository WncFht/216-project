...
这次严格课内版做了两处仍然属于课堂边界内的改动：第一，不再只用四个 case 音频当模板，而是先把训练集同类样本的最大能量块平均成类别模板，再在选定块长后用 train+dev 重新估计最终模板；第二，在 dev 集上从 3--6 帧里选块长度，最终选中 3 帧。这样做以后，strict 版本的整体正确率有了进一步提升。
