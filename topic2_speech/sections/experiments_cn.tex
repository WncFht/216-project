...
先看我们的短时 FFT 模板方案。由于任务是单标签四分类，完全匹配准确率是更直接的指标，按标签位准确率只作为辅助参考。
...
\begin{table}[H]
    ...
    \begin{tabular}{lccccc}
        ...
        \midrule
        我们的方案 & 0.6327 & 0.2655 & 0.2439 & 0.2655 & 0.2655 \\
        \bottomrule
    \end{tabular}
\end{table}
...
在我们的方案之外，本文进一步给出 AI 重做版。它不是简单更换分类器，而是重做前两步：先用 log-mel 与能量变化得到更稳的词首局部谱，再用正负模板差分和自相关完成检测。
...
\begin{table}[H]
    ...
    \begin{tabular}{lccccc}
        ...
        \midrule
        我们的方案 & 0.6327 & 0.2655 & 0.2439 & 0.2655 & 0.2655 \\
        AI 重做版 & 0.7513 & 0.5026 & 0.4974 & 0.5026 & 0.5026 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{lcccc}
        ...
        \midrule
        我们的方案 & 0.217 & 0.163 & 0.243 & 0.353 \\
        AI 重做版 & 0.464 & 0.359 & 0.429 & 0.737 \\
        \bottomrule
    \end{tabular}
\end{table}

AI 重做版的提升主要来自两点：一是把搜索范围更精确地收到了词首附近，二是把局部窗中的正负模板差分显式纳入了得分。/z/ 的提升最大，说明持续摩擦音在 log-mel 局部模板和自相关线索里更稳定；/b/ 和 /d/ 的重叠仍然明显，说明短促爆破和对齐误差仍会削弱模板证据。
...
图~\ref{fig:course-score-dist} 单独给出了 AI 重做版四个标签的测试得分分布。相比我们的方案，/z/ 的正负样本分离最明显；/g/、/b/、/d/ 仍然存在明显重叠，这也解释了为什么 AI 重做版提升明显，但没有完全消除爆破音之间的混淆。
