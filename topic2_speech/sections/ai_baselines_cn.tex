\section{扩展模板法、神经网络对照与训练曲线}

本题 bonus 要求用 AI 重做频谱描述和检测。本文把 \texttt{course\_filterbank\_corr} 作为这一路线的主体：它仍然输出可解释的局部谱模板分数，但用 log-mel filterbank、数据驱动 onset、正负模板差分和自相关替代手工 FFT 模板。MLP 和 CNN 则作为额外神经网络对照；两者都使用 train 训练、dev 选择阈值或早停、test 只做最终评测，最后用 $\arg\max$ 解码。
...
从结果上看，MLP 仍然是一个稳定的统计基线，但低于扩展模板法\texttt{course\_filterbank\_corr}。CNN 受益于更多词种和更多训练样本，exact-match 达到 0.559，高于扩展模板法的 0.503；这说明当词表覆盖上来以后，端到端谱图模型开始能学到一部分词首局部模式，但它的解释性仍弱于 \texttt{course\_filterbank\_corr}。
...
这几张图放在后面，是为了把扩展模板法和神经网络对照的角色分开：\texttt{course\_filterbank\_corr} 回答“扩展模板法如何重做频谱检测”，MLP/CNN 回答“常见学习型模型能达到什么分数”。对本文来说，真正的收获不是 CNN 有没有赢，而是课堂频谱模板、扩展模板法和神经网络模型之间的边界已经分清楚。
